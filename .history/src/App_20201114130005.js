...
export default function App() {
  ...
  const whoIsNext = () => finalQueue.length === 0 ? setLastToTakeNDoubleCola(showThis5[showThis5.length - 1]) : nextToTakeADoubleCola = setTimeout(()=> setShowThis5([finalQueue.shift(), ...showThis5]), 1000)
  ...
  return (
    <div className="App">
      ...
      <p>{showThis5.length > 0 && showThis5[finalQueue.length - 1]} </p>

      <p>{showThis5.length > 1 && showThis5[finalQueue.length - 2]} </p>

      <p>{showThis5.length > 2 && showThis5[finalQueue.length - 3]} </p>

      <p>{showThis5.length > 3 && showThis5[finalQueue.length - 4]} </p>

      <p>{showThis5.length > 4 && showThis5[finalQueue.length - 5]} </p>
    </div>
  )
}